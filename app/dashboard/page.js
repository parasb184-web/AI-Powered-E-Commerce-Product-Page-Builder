...
export default function Dashboard() {
  ...
  const stats = [
    { name: "Active Listing Drafts", value: loading ? "..." : products.length.toString(), change: "+2 this week" },
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <div>
          ...
          {loading ? (
            ...
            <div className="text-center py-20 text-slate-500 dark:text-slate-400 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl bg-white dark:bg-slate-900">
              No products found. Generate a new listing to get started!
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
              {products.map((product) => {
                const platformData = getPlatformData(product.platform);
                return (
                  <Card
                    key={product.id}
                    title={product.name}
                    description={product.generatedDescription || "No description generated yet."}
                    badge={platformData.badge}
